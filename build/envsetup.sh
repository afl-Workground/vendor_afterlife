...
function gorelease()
{
    ...
    do
        ...
        # Get the best server available from Gofile API
        local server=$(curl -s https://api.gofile.io/servers | jq -r '.data.servers[0].name')

        if [ -z "$server" ] || [ "$server" == "null" ]; then
            echo "Error: Could not retrieve Gofile server. Upload aborted."
            continue
        fi

        echo "Uploading to server (${server}.gofile.io)..."
        
        # Upload the file and capture the JSON response
        local response=$(curl -# -F "file=@$upfile" "https://${server}.gofile.io/uploadFile")
        
        # Extract the download page link using jq
        dlink=$(echo $response | jq -r '.data.downloadPage')
